...
try {
  ...
  // 04 — Tạo nhanh nhóm dịch vụ trong form Thêm
  await go("/services");
  await page.waitForTimeout(800);
  await page.locator('button:has-text("Thêm dịch vụ")').first().click();
  await page.waitForTimeout(700);
  await page.locator('input').first().fill("Trị liệu chuyên sâu"); // tên dịch vụ (để form có nội dung)
  await page.locator('button:has-text("+ Tạo nhóm mới")').first().click();
  await page.waitForTimeout(400);
  await page.locator('input[placeholder="Tên nhóm mới"]').fill("Nhóm trị liệu công nghệ cao");
  await shot("04-tao-nhanh-nhom");

  // 05 — Sửa RF để thấy dữ liệu cấu hình đã điền
  await go("/services");
  await page.waitForTimeout(800);
  await page.locator('td:has-text("RF nâng cơ mặt")').first().click();
  await page.waitForTimeout(900);
  await page.locator('button:has-text("Sửa")').first().click();
  await page.waitForTimeout(1100);
  await shot("05-sua-rf-cau-hinh");
