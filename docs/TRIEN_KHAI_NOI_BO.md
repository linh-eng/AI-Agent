...
>
> → Webapp này dùng **cổng khác: `3100`** (có thể đổi). Đây là bản DEMO nội bộ, chạy
> HTTP trong LAN (không cần HTTPS / domain).

Sau khi chạy xong:
- Nhân viên: **http://172.168.11.60:3100**
- Cổng khách: **http://172.168.11.60:3100/portal**
...
> (`/portal`) trên cùng cổng 3100. Không cần Nginx cho bản demo (có thể thêm sau).
...
COOKIE_SECURE=false
PORT=3100
APP_URL=http://172.168.11.60:3100
NEXT_PUBLIC_APP_URL=http://172.168.11.60:3100
...
```bash
# Chạy đúng cổng 3100, lắng nghe mọi IP (0.0.0.0) để LAN truy cập được:
...
> `start:lan` = `next start -H 0.0.0.0 -p 3100`. Muốn đổi cổng: sửa script `start:lan`
> trong `package.json` **hoặc** chạy `PORT=3100 pm2 start npm --name spa-demo -- start`.
...
```bash
sudo ufw allow 3100/tcp        # Ubuntu/UFW
# hoặc firewalld:
sudo firewall-cmd --permanent --add-port=3100/tcp && sudo firewall-cmd --reload
```

## 7. Kiểm tra
- Trên chính máy chủ: `curl -I http://localhost:3100/login` → `HTTP/1.1 200`.
- Từ máy khác trong LAN: mở trình duyệt **http://172.168.11.60:3100**.
...
- App này **chỉ** dùng cổng **3100** (khác 3000 và 9000) → không xung đột.
