...
/**
 * Bộ quyền NGHIỆP VỤ đầy đủ của Spa (dùng cho Quản lý; và theo quyết định phân
 * quyền của chủ: Ban điều hành + Kế toán cũng nhận toàn bộ bộ này). KHÔNG gồm
 * `user.manage` (quản trị tài khoản đăng nhập) — giữ riêng cho ADMIN vì đây là
 * trục an ninh khác (tạo/xoá tài khoản), chủ có thể yêu cầu mở thêm sau.
 */
const SPA_FULL_BUSINESS: PermissionCode[] = [
  ...CLINIC_READ,
...
/**
 * Namespace NHÂN SỰ / LƯƠNG THƯỞNG (HR-PH1..6) — TÁCH BIỆT khỏi nghiệp vụ spa.
 * Chấm công / bảng lương / hoa hồng là dữ liệu riêng tư, có chủ sở hữu riêng do
 * các phase HR quyết định. KHÔNG gộp vào SPA_FULL_BUSINESS: Kế toán/Ban điều hành
 * "toàn quyền nghiệp vụ giá & dịch vụ" KHÔNG đồng nghĩa mở toàn bộ quyền ghi lương.
 * - MANAGER (Quản lý): đủ quyền HR (đã có từ HR-PH1..6).
 * - BOD (Ban điều hành): CHỈ ĐỌC lương/chấm công/hoa hồng.
 */
const HR_MANAGER: PermissionCode[] = [
...
const dedup = (arr: PermissionCode[]): PermissionCode[] => Array.from(new Set(arr));
...
export const ROLE_PERMISSIONS: Record<RoleCode, PermissionCode[]> = {
  ...
  // Kinh doanh (Sales): CHỈ phần hành đầu ra + liên quan khách hàng — tạo khách,
  // đặt lịch, lập báo giá, CSKH, đề xuất SP. KHÔNG thiết lập dịch vụ/cơ chế/giá
  // vốn/nội bộ/MKT. (Giữ quyền kho THNG cũ cho tương thích — module kho env-gated.)
  SALES: dedup([
    ...CATALOG_READ,
    PERMISSIONS.OUTBOUND_WRITE,
    ...
    PERMISSIONS.BOOKING_WRITE,
    PERMISSIONS.TASK_WRITE,
    PERMISSIONS.PROPOSAL_WRITE, // lập báo giá (đầu ra)
    PERMISSIONS.RECOMMEND_WRITE, // đề xuất sản phẩm cho khách
    PERMISSIONS.CARE_WRITE,
    PERMISSIONS.MEDIA_WRITE,
  ]),
  ...
  // Quản lý Spa: TOÀN QUYỀN nghiệp vụ spa + tài chính + duyệt + đủ quyền HR/lương.
  MANAGER: dedup([...SPA_FULL_BUSINESS, ...HR_MANAGER]),
  ...
  // KTV (Chuyên viên): CHỈ phần hành đầu ra + liên quan khách hàng — thực hiện
  // buổi, ghi nhận vật tư, đề xuất SP cho khách, lập báo giá, chăm sóc. KHÔNG
  // thiết lập dịch vụ/công nghệ/protocol/biểu mẫu (cơ chế), KHÔNG giá vốn/nội bộ/MKT.
  SPECIALIST: [
    ...CLINIC_READ,
    PERMISSIONS.TREATMENT_WRITE,
    PERMISSIONS.CRM_WRITE,
    PERMISSIONS.TASK_WRITE,
    PERMISSIONS.RECOMMEND_WRITE, // đề xuất sản phẩm cho khách (đầu ra)
    PERMISSIONS.PROPOSAL_WRITE, // lập báo giá
    PERMISSIONS.CARE_WRITE,
    PERMISSIONS.MATERIAL_WRITE, // ghi nhận tiêu hao vật tư khi thực hiện buổi
    PERMISSIONS.MEDIA_WRITE,
  ],
