"use client";
// =============================================================================
// IA-PH2 — Workspace "Giá vốn & biên" (toàn cục, read-only). Tổng hợp giá vốn
// theo dịch vụ (version PUBLISHED / mới nhất) + link sang trang chi tiết giá vốn
// của từng dịch vụ (/services/[id]/costing) để tạo/sửa/phát hành.
// KHÔNG thêm entity/logic: chỉ COMPOSE/ĐỌC dữ liệu qua API sẵn có. Số liệu cost
// server đã mask theo finance.read (BLOCKER) — trang không tự suy giá vốn.
// =============================================================================
...
export default function ServiceCostingWorkspacePage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await apiFetch<Row[]>("/api/service-costings");
      setRows(data);
      setError(null);
    } catch (e: any) {
  ...
  return (
    <div>
      <PageHeader
        title="Giá vốn & biên"
        description="Tổng hợp giá vốn dịch vụ (theo version). Bấm dịch vụ để xem/tạo/phát hành chi tiết giá vốn."
      />

      {!canFinance && (
        <div className="mb-4 rounded-md border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-800">
          Bạn không có quyền xem dữ liệu tài chính (<code>finance.read</code>). Danh sách hiển thị nhưng số
          liệu giá vốn/biên được ẩn.
        </div>
      )}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/40 text-left text-xs uppercase tracking-wider text-muted-foreground">
                  ...
                  <th className="px-4 py-3 text-right">Giá vốn (phát hành)</th>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={7} className="px-4 py-8 text-center text-muted-foreground">Chưa có dịch vụ nào khai báo giá vốn.</td></tr>
                ) : (
                  filtered.map((r) => {
                    ...
                    return (
                      <tr key={r.serviceId} className="border-b last:border-0 hover:bg-accent/40">
                        ...
                        <td className="px-4 py-3 text-right">
                          {pub ? money(pub.totalEstimatedCost) : <span className="text-muted-foreground">Chưa phát hành</span>}
                        </td>
                        ...
                        <td className="px-4 py-3 text-right">
                          <Link href={`/services/${r.serviceId}/costing`} className="inline-flex items-center gap-1 text-sm text-primary hover:underline">
